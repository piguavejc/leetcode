...
function encontrar_objetivo(numeros, objetivo) {
    ...
    let todo_mal_objetivo = restriccion(objetivo, -Math.pow(10, 4), Math.pow(10, 4)); // [-10^4 ; 10^4] intervalo cerrado
    ...
    while (minimo <= maximo) {
        ...
        let todo_mal_actual = restriccion(actual, -Math.pow(10, 4), Math.pow(10, 4)); // [-10^4 ; 10^4] intervalo cerrado
        if (todo_mal_actual)
            return -1;
        if (objetivo === numeros[mitad])
            return mitad;
        if (objetivo < actual) {
            if (objetivo > anterior || mitad === 0)
                return mitad;
            if (mitad === 0)
                return mitad;
            maximo = mitad - 1;
