...
function juego_adivinanza(numero, objetivo) {
    ...
    let maximo = numero;
    let longitud = numero;
    let todo_mal_numero = restriccion(1, Math.pow(2, 31), longitud); /* [1 ; 2^31] */
    let todo_mal_objetivo = restriccion(0, Math.pow(10, 6), objetivo); /* [0 ; 10^6] */
    if (todo_mal_numero || todo_mal_objetivo)
        return null;
    if (guess(minimo, objetivo) === 0)
        return minimo;
    if (guess(maximo, objetivo) === 0)
        return maximo;
    while (minimo <= maximo) {
