function buscar_letra_mayor(letras: string[], letra: string): string {
    ...
    let todo_mal_letras: boolean = restriccion(2, Math.pow(10, 4), longitud); /* [2 ; 10^4] */
    ...
    if (objetivo === inicial && objetivo !== letras[minimo + 1].charCodeAt(0)) return letras[minimo + 1];
    ...
    while (minimo <= maximo) {
        ...
        if (ascii_actual === objetivo && ascii_actual !== ascii_siguiente) return siguiente;

        if (mitad > 0 && mitad < longitud) {
            let anterior: string = letras[mitad - 1];
            let ascii_anterior: number = anterior.charCodeAt(0);

            if (objetivo < ascii_actual && objetivo > ascii_anterior) return letras[mitad];

            if (ascii_anterior > objetivo && objetivo > ascii_siguiente) return letras[mitad - 1];
        }
...
function restriccion(minimo: number, maximo: number, objetivo: number): boolean {
