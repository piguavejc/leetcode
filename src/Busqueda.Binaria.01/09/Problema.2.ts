...
function busqueda_binaria(numeros: number[], objetivo: number, posicion: number): boolean {
    let longitud: number = numeros.length;
    let minimo: number = 0;
    ...
    while (minimo <= maximo) {
        ...
        let todo_mal_actual: boolean = restriccion(-Math.pow(10, 3), Math.pow(10, 3), actual); /* [-10^3 ; 10^3] */
