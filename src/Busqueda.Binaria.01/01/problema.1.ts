function buscar_objetivo(numeros: number[], objetivo: number): number {
    ...
    let todo_mal_numeros: boolean = restriccion_1(1, Math.pow(10, 4), longitud); /* [1 ; 10^4] */
    let todo_mal_objetivo: boolean = restriccion_2(objetivo, Math.pow(10, 4)); /* [objetivo ; 10^4] */
    ...
    while (minimo <= maximo) {
        ...
        let todo_mal_actual: boolean = restriccion_2(-Math.pow(10, 4), actual); /* [-10^4 ; actual] */
...
function restriccion_1(minimo: number, maximo: number, longitud: number): boolean {
