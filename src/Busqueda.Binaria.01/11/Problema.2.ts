function obtener_menor_numero(numeros: number[], objetivo: number) {
    ...
    if (inicial > numeros[minimo + 1]) return busqueda(numeros, objetivo, minimo + 1, maximo);

    /* que el menor se encuentre al final */
    if (ultimo < numeros[maximo - 1]) return busqueda(numeros, objetivo, minimo, maximo - 1);
    ...
    while (minimo <= maximo) {
        ...
        if (actual < numeros[0] && actual < numeros[longitud - 1]) maximo = mitad - 1;
        else minimo = mitad + 1;
    }

    return -1;
}
function busqueda(numeros: number[], objetivo: number, minimo: number, maximo: number): number {
    while (minimo <= maximo) {
        let mitad: number = Math.trunc((minimo + maximo) / 2);
        let actual: number = numeros[mitad];
